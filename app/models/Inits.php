...
class Inits
{
	...
	public function dbLoad(){
		$query = file_get_contents(dirname(__FILE__) . "/../lib/sf_help.sql");
		$this->db->query($query);
		if ($this->db->execute()) {
			return true;
		}
	...
	public function setSite($data){
		...
		try {
			...
			$this->db->query("INSERT INTO `sf_site` (`site_name`) VALUES (:site_name)");
			...
			*/
			...
			$this->db->query("INSERT INTO `sf_logo`(`site_fk`, `path`, `status`) VALUES ($lastInsertId, :logoPath, 1)");
